...
/*
 * Full output of lcpp_chat_templates_apply_full.
 * Caller owns this struct and must pass it to lcpp_chat_params_free().
 */
typedef struct lcpp_chat_params {
    char *prompt;         /* rendered prompt (NUL-terminated) */
    char *grammar;        /* GBNF grammar constraint, "" if none */
    int   grammar_lazy;   /* non-zero → lazy grammar application */
    char *thinking_start; /* thinking open tag, e.g. "<think>", "" if none */
    char *thinking_end;   /* thinking close tag, e.g. "</think>", "" if none */
    int   n_stops;        /* number of additional stop strings */
    char **stops;         /* n_stops C strings (plus a NULL sentinel) */
} lcpp_chat_params;

/*
 * Apply the template and return ALL fields from common_chat_params.
 * Returns NULL on error.  Must be freed with lcpp_chat_params_free().
 * Same semantics as lcpp_chat_templates_apply for enable_thinking / add_ass.
 */
lcpp_chat_params *lcpp_chat_templates_apply_full(
    lcpp_chat_templates *tmpls,
    const struct llama_chat_message *msgs,
    size_t n_msgs, int enable_thinking, bool add_ass);

/* Free a lcpp_chat_params returned by lcpp_chat_templates_apply_full(). */
